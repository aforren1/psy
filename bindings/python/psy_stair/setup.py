...
# psy_stair.h lives at the repo root; that copy always wins in a dev tree so a
...
# psy_stair.h is not a transport header and includes nothing from the
# collection, so it is the only header staged.
HEADER_DIR = REPO_ROOT if os.path.exists(os.path.join(REPO_ROOT, "psy_stair.h")) else HERE
...
# Dotted name: the extension is built as psy/stair.abi3.so. There is no
...
ext = Extension(
    name="psy.stair",
    sources=[os.path.join(HERE, "psy_stair_ext.c")],
